function [x_ITCG_vs,nv_ITCG_vs,T_ITCG_vs,iter_vs,NZ_vs]=ITCG_vs(y,A1,tau,err)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %     dianfy=z(1:N)-z(N+1:N+N); 
%     z_c=z(1:N)-z(N+1:N+N); 
%     phan_tom_poin_c=nodes;
%     phan_tom_poin_c(:,4)=z_c;
%      flag=find(phan_tom_poin_c(:,4)~=0);
%      GESHU=length(flag(:,1))
%     [phan_tom_poin_c,FLAG,center_cat] = fcmcluster1(phan_tom_poin_c, 1.7,flag);   
%     
% %      center_cat1=center_cat(:,1:3);
%      distance=10-0.01*iter;
%      [phan_tom_poin_c,dl,chazhi]=chanezhi(nodes,center_cat,phan_tom_poin_c,distance,flag,FLAG);
%      z(1:N)=2*phan_tom_poin_c(:,4);
%      z(N+1:N+N)=phan_tom_poin_c(:,4);
end
T_ITCG=cputime-t1;
