function [Label_Data,test_b]=get_double(A,test)
        ...
        for j = 1 : Len_y
            ...
            for di = 1 : len_d
                for l = 1 : Len_r
                    hh=[1 0 0];
                    %                     if norm(hh)>0
                    S_site1=S_site+hh.*(dis(di)+2*r(1,l));
                    flag_true1 = find(sum((nodes(:,1:3)-S_site).^2,2).^0.5<=r(1,l));
                    flag_true2 = find(sum((nodes(:,1:3)-S_site1).^2,2).^0.5<=r(1,l));
                    if isempty(flag_true1)  && isempty(flag_true2)
                        continue;
                    end
