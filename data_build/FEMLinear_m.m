function [  Mm ,F ] = FEMLinear_m ( fy0,sign)
%FEMLINEAR Summary of this function goes here
%   Detailed explanation goes here
global nodes elementInside elementSurface   Dm uam An 
%初始化矩阵
F  =sparse(length(nodes(:,1)),length(nodes(:,1)));
M  =sparse(length(nodes(:,1)),length(nodes(:,1)));
% fy1=zeros(length(nodes(:,1)),length(nodes(:,1)));
Mm   = FormMatrixM( M ,elementInside ,elementSurface ,nodes ,uam ,Dm ,An );
F    = FormMatrixF( F ,elementInside ,nodes,fy0,sign);
%分函数
function [ Mm ] = FormMatrixM( M ,elementInside ,elementSurface ,nodes ,uam ,Dm ,An )
% ***************************************************************
%                      将K和C加入到M中
% ***************************************************************
...
for l=1:length(elementInside(:,1))
 ...
 % 每个小四面体单元中的ai,bi,ci,di
    ...
    kl=(Dm(sign1)/(6.*D0))*[av1.*av1+bv1.*bv1+cv1.*cv1 av1.*av2+bv1.*bv2+cv1.*cv2 av1.*av3+bv1.*bv3+cv1.*cv3 av1.*av4+bv1.*bv4+cv1.*cv4
    ...
    for i = 1:4
        for j = 1:4
           M(node(i),node(j)) = M(node(i),node(j))+kl(i,j)+uam(sign1)*cl(i,j);
        end
    end
end
% *****************************************************
...
Mm=M;
function F = FormMatrixF( F ,elementInside ,nodes ,fy0,sign)
for l = 1 : length(elementInside(:,1))
    if sign(l) == 1   
    node=elementInside(l,2:5);
